...
RUN \
    ...
    if ! grep -qxF 'clear_env = no' /etc/php85/php-fpm.conf; then echo 'clear_env = no' >> /etc/php85/php-fpm.conf; fi && \
...
RUN \
    ...
    cd /app/www && \
    COMPOSER_HTTP_TIMEOUT=300 composer install --no-dev && \
    echo "**** create symlinks for linuxserver persistence ****" && \
    /bin/bash -c \
    'dst=(www/files www/images www/uploads backups www/framework/cache www/framework/sessions www/framework/views www/framework/purifier log/bookstack/laravel.log www/.env); \
    src=(storage/uploads/files storage/uploads/images public/uploads storage/backups storage/framework/cache storage/framework/sessions storage/framework/views storage/framework/purifier storage/logs/laravel.log .env); \
    for i in "${!src[@]}"; do \
    rm -rf /app/www/"${src[i]}" && \
...
# Copy custom themes directly into /app/www/themes
